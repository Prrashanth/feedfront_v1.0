...
class AnalyticsController extends CI_Controller
{
    ...
    public function Getanlytics()
    {
        ...
        $data['categorization']=$this->analytics->GetFeedbackCat($dat);
        $data['SenCat']=$this->analytics->GetCatScore($dat);
        $data['vals'] = $this->campaign->RateCalculations($dat);
		$type = array('Payments','Convenience','Customer Experience');
        for($i=0;$i<count($type);$i++)
        {
            $data[$type[$i].'Key'] = $this->analytics->GetFeedbackCatDet($dat,$type[$i]);
			$data[$type[$i].'SubCat'] = $this->analytics->GetCatScoreSub($dat,$type[$i]);
        }
	...
	public function NPSReport()
    {
        ...
        //$data['list'] = $this->campaign->FetchCampaignbyid($_GET['Campaign']);
        $data['vals'] = $this->campaign->RateCalculations($dat);
        $data['records'] = $this->analytics->NPSRepDetails($dat);
        $data['page']='analytics';
        $this->load->Loadpage('NPSReport',$data);
    }
}